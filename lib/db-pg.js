...
function pendingTransactionCount(userId) {
  return query(_txPendingCount, [Number(userId), 'pending']).then(rows => rows[0].count);
}
...
async function pruneAdminClientData(userId) {
  ...
  let n = 0;
  for (const s of stmts) n += (await query(s, [Number(userId)])).length;
  return n;
}
...
module.exports = {
  ...
  transferCash, depositCash, withdrawCash, externalTransfer,
