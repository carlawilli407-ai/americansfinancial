...
async function initDb() {
  ...
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS approved_by INTEGER REFERENCES users(id)'); } catch (_) {}
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS approved_at TIMESTAMP WITH TIME ZONE'); } catch (_) {}
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS admin_notes TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS external_bank_logo TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS external_bank_name TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS external_routing TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS external_account_holder TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE transactions ADD COLUMN IF NOT EXISTS external_account_last4 TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE profiles ADD COLUMN IF NOT EXISTS investment_objective TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE profiles ADD COLUMN IF NOT EXISTS communication_pref TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE profiles ADD COLUMN IF NOT EXISTS member_id TEXT'); } catch (_) {}
  try { await p.query('ALTER TABLE accounts ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP'); } catch (_) {}

  await hardenSupabaseApi(p);
}

// --- Fix profiles.user_id type if Supabase pre-created it as UUID ---
// Supabase auth templates may create a profiles table with user_id UUID (referencing
// auth.users). This application's users.id is SERIAL INTEGER, so a UUID user_id
// causes "invalid input syntax for type uuid" on every profile query.
// Called lazily from ensureDbReady() (server.js) so errors never crash module load.
async function fixProfilesTable() {
  try {
    const p = getPool();
    const col = await p.query(
      "SELECT data_type FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'user_id'"
    );
    if (col.rows.length > 0 && col.rows[0].data_type === 'uuid') {
      console.warn('[db-pg] profiles.user_id is UUID on remote DB — dropping & recreating with INTEGER');
      await p.query('DROP TABLE IF EXISTS profiles CASCADE');
      await p.query(
        "CREATE TABLE profiles (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL UNIQUE REFERENCES users(id) ON DELETE CASCADE, phone TEXT, address_line1 TEXT, address_line2 TEXT, city TEXT, state TEXT, zip_code TEXT, country TEXT DEFAULT 'USA', date_of_birth TEXT, ssn_last4 TEXT, employment_status TEXT, employer_name TEXT, employer_address TEXT, job_title TEXT, annual_income TEXT, net_worth TEXT, investment_experience TEXT, investment_objective TEXT, risk_tolerance TEXT, communication_pref TEXT, member_id TEXT, citizenship TEXT DEFAULT 'US', tax_id_type TEXT DEFAULT 'SSN', tax_id TEXT, created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP)"
      );
    }
  } catch (e) {
    console.error('[db-pg] profiles migration error:', e.message);
  }
}
...
async function upsertProfile(userId, data) {
  ...
  const placeholders = PROFILE_FIELDS.map((f, i) => '$' + (i + 2)).join(', ');
  return query(
    'INSERT INTO profiles (user_id, ' + cols + ') VALUES ($1, ' + placeholders + ')',
    [userId, ...vals]
  );
}
...
async function pruneAdminClientData(userId) {
  ...
  for (const s of stmts) {
    const res = await getPool().query(s, [Number(userId)]);
    n += res.rowCount || 0;
  }
  return n;
}
...
async function hardenSupabaseApi(pool) {
  ...
  for (const table of tables) {
    try {
      ...
      console.log(`[migrate] Could not enable RLS on ${table}:`, e.message);
    }
  }
