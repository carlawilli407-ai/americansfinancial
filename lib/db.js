...
`);
// --- migrations: add approval-workflow columns (idempotent) ---
(function migrate() {
...
// ---------- accounts & positions ----------
const _acctIns   = db.prepare(`INSERT INTO accounts (user_id,type,nickname,number) VALUES (?,?,?,?)`);
...
function createAccount(userId, type, nickname, number) {
  const info = _acctIns.run(userId, type, nickname || null, number);
  return info.lastInsertRowid;
}
...
const PORTFOLIO_TEMPLATE = [
  ...
  },
  {
    ...
    const _profIns = db.prepare(`INSERT INTO profiles (user_id, phone, address_line1, address_line2, city, state, zip_code, country, date_of_birth, ssn_last4, employment_status, employer_name, employer_address, job_title, annual_income, net_worth, investment_experience, risk_tolerance, citizenship, tax_id_type, tax_id)
                                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`);
    const _profUpd = db.prepare(`UPDATE profiles SET phone=?, address_line1=?, address_line2=?, city=?, state=?, zip_code=?, country=?, date_of_birth=?, ssn_last4=?, employment_status=?, employer_name=?, employer_address=?, job_title=?, annual_income=?, net_worth=?, investment_experience=?, risk_tolerance=?, citizenship=?, tax_id_type=?, tax_id=?, updated_at=datetime('now') WHERE user_id=?`);
    ...
    function upsertProfile(userId, data) {
      ...
      if (existing) {
        _profUpd.run(
          data.phone || null, data.address_line1 || null, data.address_line2 || null, data.city || null,
          data.state || null, data.zip_code || null, data.country || 'USA', data.date_of_birth || null,
          data.ssn_last4 || null, data.employment_status || null, data.employer_name || null,
          data.employer_address || null, data.job_title || null, data.annual_income || null,
          data.net_worth || null, data.investment_experience || null, data.risk_tolerance || null,
          data.citizenship || 'US', data.tax_id_type || 'SSN', data.tax_id || null, userId
        );
      } else {
        _profIns.run(
          userId,
          ...
          data.employer_address || null, data.job_title || null, data.annual_income || null,
          data.net_worth || null, data.investment_experience || null, data.risk_tolerance || null,
          data.citizenship || 'US', data.tax_id_type || 'SSN', data.tax_id || null
        );
      }
    }

    // ---------- transactions ----------
    const _txIns = db.prepare(`INSERT INTO transactions (user_id, account_id, tdate, type, symbol, description, quantity, price, amount, balance_after, status, reference_id)
                               VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`);
    ...
    function seedTransactions(userId) {
      ...
      for (const r of rows) {
        if (r.account_id) _txIns.run(userId, r.account_id, r.tdate, r.type, r.symbol, r.description, r.quantity, r.price, r.amount, r.balance_after, r.status, r.reference_id);
      }
    }
    ...
    module.exports = {
      ...
      getCashPosition, depositCash, withdrawCash,
...
function seedDefaultPortfolio(userId, initialCash) {
  for (const acc of PORTFOLIO_TEMPLATE) {
    const accId = createAccount(userId, acc.type, acc.nickname, acc.number());
