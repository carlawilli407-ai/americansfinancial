...
function buildDashboard(userId) {
  ...
  for (const acc of accounts) {
    const positions = db.listPositions(acc.id);
    let mv = 0, cv = 0, day = 0;
  ...
  const activities = db.listActivities(userId);
  const dividendsIncome = activities.filter(a => a.type === 'Dividend').reduce((s, a) => s + (a.amount || 0), 0);
  const openOrders = db.listOpenOrders(userId);
  const orderHistory = db.listOrderHistory(userId);
  const alerts = db.listAlerts(userId);
  const candles = genCandles(userId + 7, 120);
  const dbWatch = db.listWatchlists(userId);
  ...
  return {
    ...
    documents: buildDocuments(db.getUserById(userId)),
