...
// Block Supabase REST API (anon/authenticated) from public tables.
// Direct postgres connections (DATABASE_URL) bypass RLS as superuser.
async function hardenSupabaseApi(pool) {
  ...
  console.log('[migrate] Hardening Supabase API access (RLS)...');
  for (const table of tables) {
    await pool.query(`ALTER TABLE public.${table} ENABLE ROW LEVEL SECURITY`);
  }
  console.log('[migrate] RLS enabled on public tables (no policies — REST API blocked, direct postgres OK)');
}
